...
void PortScanner::ScanSinglePorts(std::string
                                  ...
                                  std::map<int, PortScanningResult>
                                  &result) {
  if (ports.empty()) {
    logger::LogInfo(fmt::format("No single ports provided for {}", host));
  } else {
...
void PortScanner::ScanPortRanges(std::string
                                 ...
                                 &result) {
  if (ports.empty()) {
    logger::LogInfo(fmt::format("No port ranges provided for {}", host));
  } else {
...
void PortScanner::ScanPort(std::string host,
                           int port,
